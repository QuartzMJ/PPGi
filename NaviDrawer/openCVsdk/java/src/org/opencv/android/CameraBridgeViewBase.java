    ...
    protected void deliverAndDrawFrame(CvCameraViewFrame frame) {
        if (screenOrientation == 2) {
            ...
            if (modified != null) {
                try {
                    Utils.matToBitmap(modified, mCacheBitmap);
                    Log.d(TAG, "Bitmap type: " + mCacheBitmap.getWidth() + "*" + mCacheBitmap.getHeight());
                } catch(Exception e) {
                    Log.e(TAG, "Mat type: " + modified);
                    Log.e(TAG, "Bitmap type: " + mCacheBitmap.getWidth() + "*" + mCacheBitmap.getHeight());
                    Log.e(TAG, "Utils.matToBitmap() throws an exception: " + e.getMessage());
            ...
            if (bmpValid && mCacheBitmap != null) {
                ...
                if (canvas != null) {
                    ...
                    if(mCameraIndex == 0) {
                        ...
                        float scale = (float) canvas.getWidth() / (float) mCacheBitmap.getHeight();
                        Log.d("Scale check", "Scale: " + Float.toString(scale) + " Width: " + Float.toString((canvas.getWidth() - mCacheBitmap.getWidth())/ 2f) + " Height: " + Float.toString((canvas.getHeight() - mCacheBitmap.getHeight())/2f ));
                        matrix.postScale(scale, scale, canvas.getWidth() / 2f, canvas.getHeight() / 2f);
                        canvas.drawBitmap(mCacheBitmap, matrix, null);
                    } else{
                        Matrix matrix = new Matrix();
...
*/
    ...
    public void setScreenOrientation(int set) {
